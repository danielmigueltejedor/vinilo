...
pub(super) fn classify(name: &str) -> Entry<'_> {
    ...
    // `<key>-512.backdrop-tone.png` — a wash of the sleeve's colour, and it
    // lives or dies with that cover. Older `backdrop256.png` files were a
    // blurred photograph of the same sleeve; they are retired, not reused.
    if let Some((size, tail)) = rest.split_once(".backdrop") {
        return if LIVE_SIZES.contains(&size) && tail == "-tone.png" {
            Entry::Cover(key)
        } else {
            Entry::Retired
        };
...
mod tests {
    ...
    fn every_shape_on_disk_is_recognised() {
        ...
        assert_eq!(
            classify("127f514fc028ac76-512.backdrop-tone.png"),
    ...
    fn the_shapes_we_stopped_writing_are_retired() {
        ...
        // size nothing else uses. The numberless backdrop and the blurred
        // photograph (`backdrop256`) predate the colour wash.
